...
from DataLoader_lmk import Dataset
from utils.utils import preprocess_img_tensor
from torch.utils import data as data_utils
from utils.model_lmk_utils import validation,PositionalEncoding
import time
import pandas as pd
from PIL import Image
import lpips
...
def collate_fn(data):
    ref_image = torch.stack([example['ref_image'] for example in data])
    image = torch.stack([example['image'] for example in data])
    landmark = torch.stack([example['landmark'] for example in data])
    masked_image = torch.stack([example['masked_image'] for example in data])
    masks = torch.stack([example['mask'] for example in data])
    audio_feature = torch.stack([example['audio_feature'] for example in data])
    return ref_image, image,landmark, masked_image, masks, audio_feature
...
def main():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        unet.train()
#         for step, batch in enumerate(train_dataloader):
        for step, (ref_image, image, landmark, masked_image, masks, audio_feature) in enumerate(train_data_loader):
            # Skip steps until we reach the resumed step
            if args.resume_from_checkpoint and epoch == first_epoch and step < resume_step:
                if step % args.gradient_accumulation_steps == 0:
                    progress_bar.update(1)
            ...
            ref_image = ref_image.to(vae.device)
            image = image.to(vae.device)
            landmark_image = landmark.to(vae.device)
            masked_image = masked_image.to(vae.device)
            ...
            with accelerator.accumulate(unet):
                ...
                # Convert landmark images to latent space
                landmark_latents = vae.encode(
                    landmark_image.reshape(landmark_image.shape).to(dtype=weight_dtype)  # ref image
                ).latent_dist.sample()
                landmark_latents = landmark_latents * vae.config.scaling_factor
                
                vae_time = time.time() - start
                start = time.time()


                mask = torch.nn.functional.interpolate(masks, size=(latents.shape[2], latents.shape[3]))
                bsz = latents.shape[0]
                # fix timestep for each image
                timesteps = torch.tensor([0], device=latents.device)
                # concatenate the latents with the mask and the masked latents
                ...
                print("masked_latents: ", masked_latents.shape)
                """
                if unet_config['in_channels'] == 12:
                    latent_model_input = torch.cat([masked_latents,landmark_latents, ref_latents], dim=1)
                else:
                    latent_model_input = torch.cat([masked_latents, ref_latents], dim=1)
